...
# Using configuration from rfid_minimal.config.config
...
def parse_arguments():
    ...
    parser.add_argument(
        "--mqtt-enabled",
        action="store_true",
        help="Enable MQTT publishing (default: True)"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=BASKET_ID,
        help=f"Set the basket ID for MQTT publishing (default: {BASKET_ID})"
    )
...
def run_rfid_system(
    ...
    try:
        ...
        for cycle in range(1, cycles + 1):
            ...
            # Publish cart data to MQTT if enabled - always publish when cart is updated
        ...
        if mqtt_available and mqtt_enabled:
            try:
                ...
                # Publish to MQTT
                logger.info(f"Publishing final cart data to MQTT: {mqtt_message}")
                publish_result = publish_message(message=mqtt_message)
                
                if publish_result:
                    logger.info("Final MQTT publish successful")
                else:
                    logger.error("Final MQTT publish failed")
            except Exception as e:
                logger.error(f"Error publishing final data to MQTT: {e}")
